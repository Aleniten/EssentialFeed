...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func insert(_ cache: [LocalFeedImage], timestamp: Date, to sut: FeedStore) -> Error? {
        ...
        sut.insert(cache, timeStamp: timestamp) { result in
    ...
    func expect(_ sut: FeedStore, toRetrieve expectedResult: FeedStore.RetrievalResult, file: StaticString = #file, line: UInt = #line) {
        ...
        sut.retrieve { retrievedResult in
            ...
            case (.success(.none), .success(.none)), (.failure, .failure):
                break
            case let (.success(cacheExpected), .success(cacheRetrieved)):
                XCTAssertEqual(cacheExpected?.feed, cacheRetrieved?.feed, file: file, line: line)
                XCTAssertEqual(cacheExpected?.timestamp, cacheRetrieved?.timestamp, file: file, line: line)
            default:
